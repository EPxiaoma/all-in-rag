"""
生成集成模块
核心职责：调用 Kimi 大模型，根据检索到的文档生成自然语言回答
"""
...
class GenerationIntegrationModule:
    ...
    def __init__(self, model_name: str = "kimi-k2.6", temperature: float = 0.1, max_tokens: int = 2048):
        """
        初始化生成集成模块
    ...
    def generate_step_by_step_answer(self, query: str, context_docs: List[Document]) -> str:
        """
        生成详细步骤回答 - 为"detail"类型查询生成分步骤的详细烹饪指导
    ...
    def query_rewrite(self, query: str) -> str:
        """
        智能查询重写 - 让 LLM 判断是否需要把模糊查询改写得更精准
    ...
    def query_router(self, query: str) -> str:
        ...
        Returns:
            三种路由类型：
            - 'list'   → 用户想要推荐列表（只需菜名，不用详细步骤）
            - 'detail' → 用户想要制作方法（需要详细分步回答）
            - 'general'→ 一般性问题（标准回答即可）
        """
        prompt = ChatPromptTemplate.from_template(
            """
    ...
    def generate_step_by_step_answer_stream(self, query: str, context_docs: List[Document]):
        """
        生成详细步骤回答 - 为"detail"类型查询生成分步骤的详细烹饪指导[流式版本]
    ...
    def _build_context(self, docs: List[Document], max_length: int = 2000) -> str:
        """
        构建上下文字符串
        1. 每个文档前加元数据头（菜名、分类、难度），帮助 LLM 定位信息
        2. max_length 限制：防止上下文太长超过模型的 context window（token 限制）
        3. 超出限制时直接停止（不截断单个文档）
